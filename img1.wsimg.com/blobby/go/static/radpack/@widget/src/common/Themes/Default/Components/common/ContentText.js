...
export default class ContentText extends Component {
        ...
        const modifiedBlocksArray = parsedCaption.blocks
            .map((block, index) => {
                if (index === blockAnchor) {
                    const shortenedCaptionText = block.text.slice(0, -(charCount - collapseThreshold));
                    return { ...block,
                        text: shortenedCaptionText
                    };
    ...
    renderCollapsedText(textProps) {
        ...
        try {
            ...
            if (!parsedCaption.blocks) {
                // Not a valid DraftJS object, ignore
                parsedCaption = null;
            }
        } catch (e) {
            // Ignore error
        ...
        const shouldCollapseCaption =
            (parsedCaption ? this.getPlainCaption(parsedCaption).length : children.length) >
        ...
        const collapsedBlock = ( <
            UX2.Element.Block id = {
                uniqueCollapsedBlock
            }
            style = {
                {
                    display: renderExpanded ? 'none' : 'block'
                }
            } >
            <
            UX2.Element.Text alignment = {
                alignment
            } { ...omit(textProps, 'textCollapseProps')
            }
            children = {
                this.getShortenedCaption(parsedCaption, children, collapseThreshold)
            }
            /> <
            UX2.Element.Label
            for = {
            ...
            <
            UX2.Element.MoreLink.Expand style = {
                {
        ...
        const expandedBlock = ( <
            ...
            style = {
                {
                    display: renderExpanded ? 'block' : 'none'
                }
            } >
            <
            UX2.Element.Text alignment = {
        ...
        return shouldCollapseCaption ? ( <
            React.Fragment > {!renderExpanded && < UX2.Element.Input { ...inputToggleProps
                }
                /> } {
                    collapsedBlock
                } {
        ...
        render() {
            ...
            const textProps = text && stringToChildren({
                style: {
                    flexGrow: 1
                },
                ...text
